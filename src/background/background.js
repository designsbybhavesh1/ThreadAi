...
console.log('🚀 ThreadAi Background Service Worker started.');
...
async function sendAIReadyNotification(source = 'download') {
    try {
        ...
        console.log(`🎉 AI ready notification sent (${source})`);
...
(async function initializeBackground() {
    ...
    if (hasOngoingDownload) {
        console.log(`📊 Restored download: ${modelDownloadProgress}% (${currentDownloadPhase})`);
    }
})();
...
async function processThreadSummarization(tab, options = {}) {
    ...
    if (!isKnownPlatform) {
        console.log(`🌐 Processing unknown platform: ${tab.url}`);
    }
    ...
    console.log('✅ Access granted:', usageCheck.reason);
            ...
            switch (request.action) {
                case 'GET_AI_AVAILABILITY':
                    ...
                    if (isModelDownloading) {
                        ...
                        if (availability === 'available') {
                            console.log('🔄 AI became available, clearing download state');
                ...
                case 'GET_DOWNLOAD_PROGRESS':
                    ...
                    if (isModelDownloading) {
                        ...
                        if (availability === 'available') {
                            console.log('🔄 AI became available during progress check, clearing download state');
                            isModelDownloading = false;
                            await clearDownloadState();
...
chrome.runtime.onStartup.addListener(async () => {
    console.log('🌅 Chrome started up, extension background is ready.');
    // Start periodic subscription checking for automatic premium activation
    PaymentService.startPeriodicSubscriptionCheck();
...
chrome.notifications.onClicked.addListener(async (notificationId) => {
    if (notificationId === 'ai-ready') {
        console.log('🎯 AI ready notification clicked, opening extension');
        try {
...
async function preInitializeAI() {
    try {
        const availability = await ChromeAIService.getAvailability();
        if (availability === 'available') {
            console.log('✅ AI available, pre-initializing...');
            // Pre-initialize session in background
            ChromeAIService.getSession().then(() => {
                console.log('🚀 AI session pre-initialized');
            }).catch(error => {
                console.warn('Pre-initialization failed:', error);
            });
        } else {
            console.warn(`⚠️ AI not available: ${availability}`);
        }
    } catch (error) {
